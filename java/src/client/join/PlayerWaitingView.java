...
//	};
	...
	public void setPlayers(PlayerInfo[] players) {
		...
		if(players.length == NUMBER_OF_PLAYERS){
			...
			labelText = ("Waiting for Players: Need " + (NUMBER_OF_PLAYERS-players.length) + " more");
		...
		for(int i = 0; i < players.length; i++){
			String builtString = (i+1) + " " + players[i].getName();
			...
			playerPanel.setBackground(players[i].getColor().getJavaColor()); //set the background color of the player
